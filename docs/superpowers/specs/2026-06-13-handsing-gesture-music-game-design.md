...
- **停留確認(dwell,2026-06-14 兩段、UI 可調)**:進入 pad 須停留 ≥ dwell 才發聲 → 快速經過不誤觸。分「換音」(`KEY_DWELL_DIFF_MS`,較長,過濾掃過其他音)與「同音重觸」(`KEY_DWELL_SAME_MS`,較短,連打同音跟手),mapper 記 `lastPlayed` 判斷是否同音;兩段皆可由 UI 即時調(靈敏/中/鈍)。代價:dwell 越大發聲延遲越大。
- **排列模式(2026-06-14 UI 可切)**:`thirds`(2D 三度,預設,移動不經過其他鍵)與 `row`(單排鍵盤,直覺但會經過中間音);`melodyGeom(mode)` 解析幾何,mapper/renderer 以 `setKeyboard` 執行期切換。
...
- **頂部玻璃控制列**:LIVE / 名稱 / 音階 preset / 調 / **排列(三度 / 鍵盤)** / 伴奏律動 + BPM / 音色 / **換音靈敏** / **同音靈敏**。
